                              ...
                              int64_t *percentiles)
{
    for (size_t i = 0; i < N_MEASURES; i++) {
        ...
        t_push(tt[0], difference, classes[i]);

        for (size_t crop_index = 0; crop_index < DUDECT_NUMBER_PERCENTILES;
             crop_index++) {
            if (difference < percentiles[crop_index]) {
                t_push(tt[crop_index + 1], difference, classes[i]);
            }
        }

        if (tt[0]->n[0] > 10000) {
            double centered = (double) difference - tt[0]->mean[classes[i]];
            t_push(tt[1 + DUDECT_NUMBER_PERCENTILES], centered * centered,
                   classes[i]);
        }
    }
}
...
static t_context_t *max_test()
{
    ...
    for (size_t i = 0; i < DUDECT_TESTS; i++) {
        if (tt[i]->n[0] > ENOUGH_MEASURE) {
            double x = fabs(t_compute(tt[i]));
            if (max < x) {
                max = x;
                ret = i;
            }
